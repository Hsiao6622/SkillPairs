...
/**
 * Servlet implementation class PairMain2
 */
@WebServlet("/PairMain2")
public class PairMain2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter out = response.getWriter()) {
			if ((boolean) request.getSession().getAttribute("login") == true) {
				...
				stmtdelete.executeUpdate("DELETE FROM room WHERE FirstUserID='"
						+ request.getSession().getAttribute("UID") + "' and RoomID=(select * from (SELECT max(RoomID) FROM room)as r)"); // here
